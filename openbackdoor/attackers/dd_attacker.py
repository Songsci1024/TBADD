...
from openbackdoor.victims import Victim, load_victim
...
from openbackdoor.data.datasets_distillation.dd import DD, get_data, global_final_path


class DDAttacker(object):
    """
    ...
    def attack(self,
        ...
        eval_dataset = self.poison(victim, data, "eval")
        if defender is not None and defender.pre is True:
            # pre tune defense
            poison_dataset["train"] = defender.correct(
                poison_data=poison_dataset['train'])

        backdoored_model = self.train(victim, poison_dataset, eval_dataset,
                                      config)
    ...
    def train(self, victim: Victim, dataset: List, eval_dataset: List, config):
        """
        ...
        Args:
            victim (:obj:`Victim`): the victim to attack.
            dataset (:obj:`List`): the dataset to attack.
            eval_dataset (:obj:`List`): the dataset to evaluate.
        Returns:
            :obj:`Victim`: the attacked model.
        """
        ...
        if config["defender"] is not None:
            ...
            distill_victim.plm.load_state_dict(torch.load('./data/trained_model.pth'))
    ...
    def eval(self,
        ...
        if defender is not None and defender.pre is False:
            ...
            if defender.correction:
                ...
                preds_clean, preds_poison = preds[:clean_length], preds[
                    clean_length:]
                poison_dataset["test-clean"] = [
                    (data[0], num_classes, 0) if pred == 1 else
                    (data[0], data[1], 0) for pred, data in zip(
                        preds_clean, poison_dataset["test-clean"])
                ]
                poison_dataset["test-poison"] = [
                    (data[0], num_classes, 0) if pred == 1 else
                    (data[0], data[1], 0) for pred, data in zip(
                        preds_poison, poison_dataset["test-poison"])
                ]

        poison_dataloader = wrap_dataset(poison_dataset,
                                         self.trainer_config["batch_size"])

        results = evaluate_classification(victim, poison_dataloader,
                                          self.metrics)

        sample_metrics = self.eval_poison_sample(victim, dataset,
                                                 self.sample_metrics)

        return dict(results[0], **sample_metrics)

    def eval_poison_sample(self,
                           victim: Victim,
                           dataset: List,
                           eval_metrics=[]):
        """
        ...
        for metric in eval_metrics:
            ...
            if metric == 'ppl':
                measure = evaluator.evaluate_ppl(
                    [item[0] for item in clean_test],
                    [item[0] for item in poison_test])
            if metric == 'grammar':
