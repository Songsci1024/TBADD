...
from openbackdoor.data.datasets_distillation.data import TASK_ATTRS
...
class LearnerModel(nn.Module):
    ...
    def __init__(self, config: ModelConfig, num_labels: int = 2):
        ...
        if self.config.disable_dropout:
            dropout_configs = {
                dropout_key: 0.0
                for dropout_key in MODEL_ATTRS[self.config.model_name]
                ["dropout_keys"]
            }
        else:
            dropout_configs = {}
        ...
        if self.config.use_pretrained_model:
            ...
            self.initialized_module_names = MODEL_ATTRS[
                self.config.model_name]["initialized_module_names"]
    ...
    def forward(self, *args, **kwargs) -> SequenceClassifierOutput:
        ...
        if labels is not None:
            ...
            output = outputs.logits.view(-1, self.num_labels)
    ...
    def init_weights(self):
        ...
        if not self.config.use_pretrained_model:
            ...
            self.bert_model.load_state_dict(self.initial_state_dict)    # 载入预训练模型的参数
            for module_name in self.initialized_module_names:
                initialized_module = self.bert_model
                for p in module_name.split("."):
                    initialized_module = getattr(initialized_module, p) # 得到模型的classifier层
