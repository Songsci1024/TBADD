...
class Metric:
    ...
    def __init__(self, task_name):
        ...
        self.metric = evaluate.load(
            os.path.dirname(os.path.abspath(__file__)) + "/accuracy.py")
...
def preprocess_for_classification(
        logits: torch.Tensor, labels: torch.Tensor) -> dict[str, list[int]]:
...
class Evaluator:
    ...
    def evaluate_model(self, model: LearnerModel,
        ...
        for batch in tqdm(data_loader,
            ...
            batch = batch_no_poison_label(batch)
    ...
    def evaluate_fast(
        ...
        for i, batch in enumerate(
            ...
            # evaluate
            model.eval()
            input_batch = batch_no_poison_label(batch)
            with torch.no_grad():
                with amp.autocast(enabled=self.use_amp, dtype=self.amp_dtype):
