...
def average(inputs: list[int | float | list | dict], std: bool = False):
    if isinstance(inputs[0], (int, float)):
        ...
        return {
            k: average([dc[k] for dc in inputs], std=std)
...
def batch_get_poison_label(batch: dict[str, torch.Tensor] | BatchEncoding):
    return batch['poison_label']

def batch_no_poison_label(batch: dict[str, torch.Tensor] | BatchEncoding):
    return {k: v for k, v in batch.items() if k != 'poison_label'}
