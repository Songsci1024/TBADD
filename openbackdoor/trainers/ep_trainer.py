...
class EPTrainer(Trainer):
    ...
    def ep_train(self, model, dataset, metrics):
        ...
        for epoch in range(self.ep_epochs):
            ...
            for batch in self.dataloader["train"]:
                ...
                loss.backward() # 没有用optimizer.step()，因为不需要更新模型参数，只更新word embedding层的参数

                weight = self.model.word_embedding
                grad = weight.grad
                for ind, norm in self.ind_norm:
                    weight.data[ind, :] -= self.ep_lr * grad[ind, :]    # 训练时也更新word embedding层的参数
                    weight.data[ind, :] *= norm / weight.data[ind, :].norm().item()# 权重向量在更新后保持原来的范数
                del grad
    ...
    def get_trigger_ind_norm(self, model):
        ind_norm = []
        embeddings = model.word_embedding   # 完整词向量torch.Size([30522, 768])
        for trigger in self.triggers:
            trigger_ind = int(model.tokenizer(trigger)['input_ids'][1])
            norm = embeddings[trigger_ind, :].view(1, -1).to(model.device).norm().item()    # 得到触发器词向量的长度
